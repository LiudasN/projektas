#include "funkcijos_1.h"
void getAverages(list<mokinys>& p) {
    auto start = std::chrono::steady_clock::now();
    cout << "Skaiciuojami vidurkiai" << endl;
    double sum = 0;
...
void getMedians(list<mokinys> &p) {
    cout << "Skaiciuojama mediana..." << endl;
    auto start = std::chrono::steady_clock::now();
...
void sortByCool(list<mokinys>& l, list<mokinys>& p) {
    auto start = std::chrono::steady_clock::now();
    cout << "Dalijama i grupes..." << endl;
    for (int i = 0; i < p.size(); i++) {
        if (p[i].vidurkis <= 5) {
            l.push_back(p[i]);
            p.erase(p.begin() + i);
...
void writeEverything(list<mokinys> p, list<mokinys> l) {
    auto start = std::chrono::steady_clock::now();
    cout << "Pildoma..." << endl;
